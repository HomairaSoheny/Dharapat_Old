...
from pbl_cib_analytics_1st_fb.utils.parsing_utils import type_converters
...
class CIB:
    ...
    def __init__(self, json_obj):
        ...
        try:
            ...
            self.cib_id = "id"
            self.cib_category = str((np.random.choice(["Type j","Type k", "Type b"])))#(np.random.choice(["Type a", "Type b", "Type c", "Type d", "Type e", "Type f", "Type g", "Type h", "Type i"]))
        try:
        ...
        self.summary_1B = parsing_helpers.parse_facility_table(self.cib_raw['1.(B) SUMMARY OF THE NON-FUNDED FACILITIES AS BORROWER & CO-BORROWER'])
        ...
        self.summary_2B = parsing_helpers.parse_facility_table(self.cib_raw['2.(B) SUMMARY OF THE NON-FUNDED FACILITIES AS GUARANTOR'])
...
def standardize_date_column(df):
    ...
    # Optional: Convert to datetime format
    if 'Date' in df.columns:
        df['Date'] = pd.to_datetime(df['Date'], dayfirst=True, errors='coerce')
    #else:
    #    raise ValueError("The DataFrame does not contain a 'Date' or 'Accounting Date' column.")
