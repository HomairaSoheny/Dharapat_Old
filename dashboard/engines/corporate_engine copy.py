...
def getEndDateOfContract_1(fac):
...
def getEndDateOfContract(fac):
    ...
    return pd.NaT  # Return NaT if no valid end date is found
...
def getOutstandingZeroDate_1(fac):
...
def getSummaryOfExpiredButShowingLiveFundedTotalSum( total_type, total_df):
    return {
        "Nature of Facility": total_type,
        "Limit": (total_df["Limit"].sum()),
        "Outstanding": (total_df['Outstanding'].sum()),
        "Overdue": (total_df["Overdue"].sum()),
        "Start Date": "-",
        "End Date of Contract": "-",
        "Installment Amount": "-",
        "Payment Period": "-",
